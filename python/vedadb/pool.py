...
class PoolHealthStatus:
    ...
    dead_connections_removed: int = 0
...
class ConnectionPool:
    ...
    Features:
      ...
      - **Periodic pool health:** Background thread pings all idle
        connections every *health_check_interval* and removes dead ones.
    ...
    def _health_check_loop(self, interval: float) -> None:
        """Background thread: ping idle connections and remove dead ones."""
        while not self._closed:
            time.sleep(interval)
            if self._closed:
                break
    ...
    def _run_health_checks(self) -> None:
        """Ping all idle connections and remove unresponsive ones."""
        with self._condition:
            to_check = list(self._pool)
            alive: deque[_PoolConnection] = deque()
            dead: list[_PoolConnection] = []
        ...
        for conn in dead:
            conn.close()
            logger.debug("Health check removed dead connection")

        if dead:
            logger.info(
                "Pool health check: removed %d dead connection(s), %d idle remaining",
                len(dead),
                len(self._pool),
            )
        else:
            logger.debug(
                "Pool health check: all %d idle connections healthy",
                len(self._pool),
            )
    ...
    def health_status(self) -> PoolHealthStatus:
        """Return the current health status of the pool.

        This includes information about idle connections, dead connections
        that have been removed, and overall pool health.
        """
        with self._condition:
            total = len(self._pool) + len(self._checked_out)
            return PoolHealthStatus(
                healthy=total > 0,
                total_connections=total,
                idle_connections=len(self._pool),
                active_connections=len(self._checked_out),
                failed_health_checks=self._failed_health,
                last_health_check=self._last_health_check,
                dead_connections_removed=self._dead_removed,
            )

    def run_health_checks(self) -> int:
        """Manually trigger a health check pass on all idle connections.

        Returns:
            Number of dead connections removed.
        """
        previous_dead = self._dead_removed
        self._run_health_checks()
        return self._dead_removed - previous_dead
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"health={self.health_status.healthy}>"
        )
